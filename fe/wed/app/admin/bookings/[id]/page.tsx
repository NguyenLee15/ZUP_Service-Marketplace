...
export default function AdminBookingDetailPage() {
  ...
  const fetchBookingDetail = () => {
    ...
    Promise.all([
      ...
      adminApi.getBookingTimeline(Number(id)).catch(() => null),
    ])
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-3">
              ...
              <Badge
                className={`${st.color} border text-xs font-bold px-2.5 py-0.5`}
              >
                {st.label}
              </Badge>
            </div>
              ...
              <CardContent className="pt-4 space-y-3.5 text-sm">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="bg-white/50 p-3 rounded-xl border border-blue-100/20">
                    ...
                    <span className="font-semibold text-amber-600 mt-1 block">
            ...
            <Card className="border-red-100 bg-red-50/10 rounded-2xl shadow-sm">
              <CardHeader className="pb-3 border-b border-red-100/40">
                <CardTitle className="text-sm font-bold text-rose-700 flex items-center gap-1.5">
                  <ShieldAlert className="w-4 h-4 text-rose-600" /> Tranh chấp
                  khiếu nại đang mở
                </CardTitle>
              </CardHeader>
              <CardContent className="pt-4 space-y-3.5 text-sm">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="bg-white p-3 rounded-xl border border-red-100/30">
                    ...
                    <span className="font-semibold text-slate-700 mt-1 block">
                      {formatDate(booking.dispute.createdAt)}
                    </span>
                  </div>
                </div>
                  ...
                  <div className="bg-emerald-50/40 border border-emerald-100 rounded-xl p-3">
                    ...
                    {booking.dispute.resolutionAction && (
                      <Badge className="mt-2 border-0 bg-emerald-100 text-emerald-800 font-bold text-[10px]">
                        {booking.dispute.resolutionAction === "COMPLETE"
                          ? "Hoàn thành"
                          : "Phạt thợ"}
                      </Badge>
                    )}
                  ...
                  {statusHistory.map((h: ApiPayload, i: number) => {
                    ...
                    return (
                      ...
                      >
                        {/* Circle marker */}
                        <div
                          className={`absolute -left-[31px] top-1.5 w-3.5 h-3.5 rounded-full border-4 border-white shadow-sm flex items-center justify-center transition-transform group-hover:scale-110 ${
                            h.toStatus === "DONE"
                              ? "bg-emerald-500"
                              : h.toStatus === "DISPUTED"
                                ? "bg-rose-500"
                        ...
                        <div className="flex flex-col gap-0.5">
                          <div className="flex items-center gap-2 flex-wrap">
                            <span
                              className={`px-2 py-0.5 rounded-md text-[10px] font-extrabold uppercase border ${stepStatus.color}`}
                            >
                              {stepStatus.label}
                            </span>
        ...
        <DialogContent className="w-full max-w-md border-slate-200 rounded-2xl shadow-xl">
          ...
          <div className="space-y-4">
              <p className="text-xs text-slate-500 leading-relaxed">
                ...
                cung cấp sẽ nhận được thông báo.
              </p>

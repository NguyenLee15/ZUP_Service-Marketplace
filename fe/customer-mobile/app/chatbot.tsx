...
const EMPTY_SUGGESTIONS: ChatbotQuickReply[] = [
  ...
  { label: '🧼 So sánh vệ sinh', message: 'So sánh các dịch vụ vệ sinh nhà giúp tôi' },
  { label: '📦 Kiểm tra đơn hàng', message: 'Kiểm tra tình trạng đơn hàng gần đây của tôi' },
  { label: '🔄 Đặt lại dịch vụ', message: 'Tôi muốn đặt lại dịch vụ đã dùng trước đó' },
];

function normalizeChatbotResponse(payload: unknown): NormalizedChatbotResponse {
  const data: any = unwrapData(payload);
  return {
    reply: data?.reply || data?.message || data?.answer || 'Mình đã ghi nhận yêu cầu của bạn.',
...
function getActionTarget(action?: ChatbotAction, citations?: ChatbotCitation[]) {
  if (!action) return null;
  const payload = action.payload || {};
  const bookingId = Number(payload.bookingId || payload.id);
  const serviceId = Number(payload.serviceId);
  if (action.type === 'VIEW_BOOKING' && Number.isFinite(bookingId) && bookingId > 0) {
    return routes.booking.detail(String(bookingId));
  }
  if (Number.isFinite(serviceId) && serviceId > 0 && action.type === 'OPEN_PROVIDER_CHAT') {
    return { serviceId };
  }
  const bookingCitation = citations?.find((item) => item.type === 'booking' && item.id);
  if (bookingCitation?.id) return routes.booking.detail(String(bookingCitation.id));
  return (action.href as any) || null;
}

function messageFromResponse(response: NormalizedChatbotResponse): ChatbotMessage {
  return {
...
export default function ChatbotScreen() {
  ...
  const [chatLoadingServiceId, setChatLoadingServiceId] = useState<string | number | null>(null);
  ...
  const quickReplies = items.length === 0 ? EMPTY_SUGGESTIONS : lastAssistantQuickReplies;
    ...
    try {
      const response = normalizeChatbotResponse(
        await chatbotApi.ask({
          ...
          sessionId,
          history: items.slice(-8).map((item) => ({ role: item.role, content: item.content })),
          pageContext: buildPageContext(),
          confirmedActionId,
        }),
      );
      if (response.sessionId) setSessionId(response.sessionId);
      setItems((current) => dedupeChatMessages([...current, messageFromResponse(response)]));
  ...
  const loadSession = async (nextSessionId: string) => {
    ...
    setLoading(true);
    try {
      const history = normalizeHistoryMessages(await chatbotApi.getSessionMessages(nextSessionId));
  ...
  const openServiceChat = async (service: ChatbotService) => {
    ...
    try {
      ...
      router.push(routes.chatRoom(conversationId, service.providerName || 'Nhà cung cấp'));
    } catch (err) {
  ...
  const handleAction = async (action: ChatbotAction, citations?: ChatbotCitation[]) => {
  ...
  return (
    ...
    >
      <FlashList
          ...
          <View style={styles.headerWrap}>
            ...
            {error ? (
              <View style={styles.errorBlock}>
                ...
                {lastUserMessage ? (
                  <Button mode="outlined" icon="refresh" onPress={retryLast} style={styles.roundedButton}>
                    Thử lại
                  </Button>
                ) : null}
              </View>
        ...
        ListFooterComponent={loading ? <LoadingBubble /> : <View style={{ height: 8 }} />}
        renderItem={({ item }) => (
          <MessageBubble
            item={item}
      ...
      <View style={styles.inputShell}>
        ...
        <View style={styles.inputRow}>
          ...
          <Pressable
            ...
            style={[styles.sendButton, (!message.trim() || loading) ? styles.sendButtonDisabled : null]}
          >
  ...
  return (
    <View style={styles.messageWrap}>
      <View style={styles.aiMessageHeader}>
        ...
        <CustomerCard style={[styles.bubble, styles.assistantBubble, { paddingVertical: 12 }]}>
...
const styles = StyleSheet.create({
  ...
  assistantBubble: { backgroundColor: '#ECFDF5', borderColor: '#D1FAE5', borderWidth: 1 },
  ...
  serviceImage: { width: 78, height: 78, borderRadius: 12, backgroundColor: Colors.light.surfaceVariant },
  imageFallback: {
    width: 78,
    height: 78,
    borderRadius: 12,

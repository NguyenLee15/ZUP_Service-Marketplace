...
export const chatbotApi = {
  ask: (payloadOrMessage: ChatbotAskPayload | string, pageContext?: Record<string, unknown>) => {
    const payload =
  ...
  prepare: (payloadOrContext?: ChatbotAskPayload | Record<string, unknown>) => {
    const payload =
      payloadOrContext && ('message' in payloadOrContext || 'sessionId' in payloadOrContext)
        ? payloadOrContext
        : { pageContext: payloadOrContext };
    return api.post('/chatbot/prepare', payload);
  },
  getSessions: () => api.get('/chatbot/sessions'),
  getSessionMessages: (id: string) => api.get(`/chatbot/sessions/${id}/messages`),

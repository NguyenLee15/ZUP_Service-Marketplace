...
export function AuthCard({ title, description, children, className }: AuthCardProps) {
  return (
    <Card className={cn('w-full overflow-hidden rounded-2xl border border-slate-200/80 bg-white py-0 shadow-sm dark:border-slate-800 dark:bg-slate-900', className)}>
      <CardHeader className="space-y-1.5 p-5 pb-2 text-left sm:p-8 sm:pb-3">
        <h1 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-slate-100">
          {title}
        </h1>
        {description && (
          <CardDescription className="text-xs sm:text-sm leading-relaxed text-slate-600 dark:text-slate-400">
            {description}
          </CardDescription>
        )}
      </CardHeader>
